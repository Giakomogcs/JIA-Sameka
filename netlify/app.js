...
const RAG_DOCS_LIST_URL = `${API_BASE}/sameka-rag-docs`;
const RAG_DOCS_DELETE_URL = `${API_BASE}/sameka-rag-doc-delete`;
const RAG_PURGE_URL = `${API_BASE}/sameka-rag-purge-all`;
...
function toggleLogin(loggedIn) {
  ...
  if (!loggedIn)
    setTimeout(() => elements.emailInput && elements.emailInput.focus(), 100);
}
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          showLoginError("E-mail não confirmado. Contate o administrador.");
        } else if (
...
async function fetchSessions() {
  try {
    ...
    console.log("[Sameka] fetchSessions →", sessionsUrlWithUser);
    const response = await fetch(sessionsUrlWithUser, {
      cache: "no-store",
    });

    if (!response.ok) {
      console.error("[Sameka] fetchSessions HTTP", response.status);
      throw new Error("Failed to fetch sessions");
    }

    const text = await response.text();
    console.log("[Sameka] fetchSessions raw:", text.slice(0, 300));
    ...
    try {
      ...
      console.error("[Sameka] fetchSessions JSON parse error", e);
      return [];
    }
    ...
    console.error("[Sameka] fetchSessions error", error);
    return [];
  }
}
async function fetchHistory(sessionId) {
  try {
    const url = `${HISTORY_URL}?sessionId=${sessionId}`;
    console.log("[Sameka] fetchHistory →", url);
    const response = await fetch(url);
    if (!response.ok) {
      console.error("[Sameka] fetchHistory HTTP", response.status);
      throw new Error("Failed to fetch history");
    }

    const text = await response.text();
    console.log("[Sameka] fetchHistory raw:", text.slice(0, 300));
    if (!text || text.trim().length === 0) {
      return [];
    }
    ...
    try {
      ...
      console.error("[Sameka] fetchHistory JSON parse error", e);
      return [];
    }
    ...
    return data.map((item) => {
      ...
      const newMatch =
        processedItem.content &&
        processedItem.content.match(/^Arquivo (.+) enviado para an?lise\.$/);
    ...
    console.error("[Sameka] fetchHistory error", error);
    return [];
  }
}
...
async function sendMessage(message, sessionId, onChunk) {
  ...
  try {
    ...
    const userCidades = (currentUserMeta.cidades || []).join(", ") || "Todas";
      ...
      for (const line of lines) {
        if (line.startsWith("data: ")) {
          ...
          try {
            ...
            const newText = data.output || data.message || data.text || "";
            if (newText) {
              if (
...
function getSocialIcon(url) {
  ...
  if (u.includes("wa.me") || u.includes("whatsapp")) return "message-circle";
  return "globe";
}
...
function getSocialClass(url) {
  ...
  if (u.includes("facebook") || u.includes("fb.com")) return "social-facebook";
  if (u.includes("wa.me") || u.includes("whatsapp")) return "social-whatsapp";
...
function _normalizeLead(l) {
  ...
  out.telefones = _asLeadArray(l.telefones != null ? l.telefones : l.telefone);
  out.emails = _asLeadArray(l.emails != null ? l.emails : l.email);
  // Redes sociais + sites institucionais. A API Oporttuna devolve `sites`
  // (URLs de site/loja) separado de `redesSociais`; juntamos os dois aqui para
  // renderizar ambos na seção "Redes Sociais" (links rotulados "Site" para
  // URLs genéricas).
  out.redesSociais = [
    ..._asLeadArray(l.redesSociais != null ? l.redesSociais : l.redes),
    ..._asLeadArray(l.sites != null ? l.sites : l.site),
  ].filter((v, i, a) => v && a.indexOf(v) === i);
  // Imagens: a API Oporttuna devolve `fotos` (Google Places); o catálogo pode
  // usar `imagens`/`images`. Aceitamos todos os aliases.
  out.imagens = _asLeadArray(
    l.imagens != null ? l.imagens : l.images != null ? l.images : l.fotos,
  );
  if (out.possuiPresencaDigital == null) {
    ...
    if (typeof pd === "boolean") out.possuiPresencaDigital = pd ? "SIM" : "NAO";
...
function _normalizeLeads(payload) {
  ...
  if (arr && !Array.isArray(arr) && typeof arr === "object") {
    arr =
      arr.leads || arr.empresas || arr.data || arr.results || arr.items || [];
  }
...
function renderLeadCards(leads, priorKeys) {
  ...
  const tierRank = (l) => {
    const c = String((l.score && l.score.classificacao) || "").toUpperCase();
  ...
  if (clientes.length && prospects.length) {
    ...
    nCli = Math.min(clientes.length, MAX_LEADS_PER_RENDER - nPro);
    const intercalado = [];
    let i = 0;
    while (
  ...
  let html = repeatNote + limitNote + '<div class="lead-cards-container">';
  visiveis.forEach((lead, idx) => {
    ...
    const hasScore =
      lead.score && (lead.score.nota !== null || lead.score.classificacao);
...
function processLeadBlocks(container) {
  const codeBlocks = container.querySelectorAll("code.language-sameka-leads");
...
function processProductImages(container) {
  ...
  imgs.forEach((img) => {
    ...
    const isSamekaProduct = /sameka\.nimiam\.com\.br|nimiam/i.test(src);
    const isImage = /\.(jpe?g|png|webp|gif)(\?|$)/i.test(src);
    if (!isSamekaProduct && !isImage) return;
    if (img.closest(".product-card") || img.closest(".lead-carousel")) return;

    const card = document.createElement("span");
    card.className = "product-card";
    const newImg = img.cloneNode(true);
    newImg.setAttribute("loading", "lazy");
    newImg.setAttribute("onerror", "this.parentElement.style.display='none'");
  ...
  while (i < cards.length) {
    ...
    while (next && next.classList && next.classList.contains("product-card")) {
      group.push(next);
...
function setStatus(message, type = "") {
  ...
  elements.statusMessage.style.color =
    type === "error" ? "#d32f2f" : type === "success" ? "#388e3c" : "#7A1818";
...
async function handleSendMessage(e) {
  ...
  const response = await sendMessage(message, currentSessionId, (chunkText) => {
...
async function onEstadosChanged() {
  ...
  cidadesOpts.innerHTML = '<div class="ms-loading">Carregando cidades...</div>';
  if (placeholder) {
  ...
  ufsToLoad.sort();
  const results = await Promise.all(ufsToLoad.map((uf) => fetchCidades(uf)));
...
function renderGroupedCidades(filter) {
  ...
  cidadesGroupedData.forEach((group) => {
    ...
    filteredCidades.forEach((c) => {
      ...
      cb.type = "checkbox";
      cb.checked =
        selectedCidades.includes("TODAS") || selectedCidades.includes(val);
      div.appendChild(cb);
...
function hideUsersPage() {
  ...
  if (chatArea && (!ragPage || ragPage.style.display === "none")) {
...
async function loadUsersPage() {
  ...
  try {
    ...
    data.forEach((u) => {
      ...
      } else if (uEstados.includes("TODOS") && uCidades.includes("TODAS")) {
        coverageTxt = '<span style="font-weight:600">Todo o Brasil</span>';
      } else {
      ...
      if (delBtn) delBtn.addEventListener("click", () => openDeleteUser(u));
...
function openCreateUser() {
  ...
  const tags = cidadesTrigger ? cidadesTrigger.querySelector(".ms-tags") : null;
  if (tags) tags.remove();
  ...
  try {
    ...
    tbody.querySelectorAll("button[data-rag-doc-id]").forEach((btn) => {
      btn.addEventListener("click", () =>
        confirmDeleteRagDoc(btn.dataset.ragDocId, btn.dataset.ragDocTitle),
      );
    });
  } catch (err) {
    ...
    if (empty) {
      ...
      empty.innerHTML =
        '<div style="font-size:2rem;margin-bottom:8px">⚠️</div>Falha ao carregar documentos.<br>Verifique se o endpoint <code>/sameka-rag-docs</code> está ativo no n8n.';
    }
  } finally {
    if (loading) loading.style.display = "none";
  }
...
async function uploadRagDoc(file) {
  if (!file) return;
  try {
    showRagBusy(`Indexando "${file.name}"... isso pode levar alguns minutos.`);
...
function showRagBusy(message) {
  ...
  ["ragDocUploadBtn", "ragDocsRefreshBtn", "ragPurgeAllBtn"].forEach((id) => {
...
if (ragDocUploadBtn && ragDocFileInput) {
  ragDocUploadBtn.addEventListener("click", () => ragDocFileInput.click());
    ...
    try {
      if (editingUserId) {
        ...
        const { error } = await supabaseClient.rpc(USER_RPC.UPDATE, {
          ...
          p_full_name: document.getElementById("formFullName").value.trim(),
          p_role: chosenRole,
